...
Squares.prototype = {
	init : function(params) {
		...
		this.started2 = false;
		this.square_size2 = 50;
		this.min_square_size2 = 35;
		this.max_square_size2 = 150;
		this.direction2 = 0;

		this.speed2 = 20;
		this.grow_squares2 = 1;
		this.grow_factor2 = 3;
		this.circle_grow_factor2 = 1;
		this.move_factor2 = 5;

		this.left_offset2 = 70;
		this.top_offset2 = 70;

		this.top_extra2 = 10;
		this.enable_circles2 = 0;
		this.circle_size2 = 0;
		this.circle_colour2 = 0;
	...
	setup2 : function() {
		...
		this.width2 = window.innerWidth*2;
		this.height2 = window.innerHeight*2;

		var x=0;
		for (var j=0; j<this.height2 + (this.square_size2 * this.top_extra2); j+=this.square_size2) {
			var offset = (x%2 == 0) ? this.square_size2 : 0;
			x++;

			this.squares2[j] = [];
			this.circles2[j] = [];

			for (var i=offset; i<this.width2; i+=this.square_size2*2) {
				var topv = j - (this.square_size2 * this.top_extra2);
				var e = document.createElementNS('http://www.w3.org/2000/svg','rect');
				e.setAttribute('x',i);
				e.setAttribute('y',topv);
				e.setAttribute('width',this.square_size2);
				e.setAttribute('height',this.square_size2);
				e.setAttribute('style','fill: '+this.colour4.get());
				this.svg.appendChild(e);
				this.squares2[j][i] = e;
	...
	cycle2 : function() {
		...
		if (this.direction2 == 0) {
			this.square_size2+= this.grow_factor2;
			this.left_offset2-= this.move_factor2;
			this.top_offset2-= this.move_factor2;

			if (this.enable_circles2) {
				this.circle_size2+=this.circle_grow_factor2;
			}

			if (this.square_size2 >= this.max_square_size2) {
				this.direction2 = 1;
				this.colour3.cycle(100);
			}
			...
			if (this.square_size2 <= this.min_square_size2) {
				this.direction2 = 2;
				this.colour3.cycle(100);

				this.cycle_count2++;

				if (this.cycle_count2 %3 == 0) {
					this.grow_squares2 = 0;
				} else {
					this.grow_squares2 = 1;
				}

				if (this.cycle_count2 %4 == 0) {
					this.circle_colour2 = (this.circle_colour2 ? 0 : 1);
					this.svg.style.backgroundColor = this.circle_colour2 ? '#fff' : '#000';
				}
			}
		} else if (this.direction2 == 2) {
			this.square_size2+= this.grow_factor2;
			this.left_offset2-= this.move_factor2;
			this.top_offset2+= this.move_factor2;
			...
			if (this.square_size2 <= this.min_square_size2) {
				...
				this.direction2 = 0;
				this.enable_circles2 = (this.enable_circles2 ? 0 : 1);
		...
		var x2=0;
		for (var j in this.squares2) {
			var x = (x2%2 == 0) ? this.square_size2 : 0;
			x2++;

			for (var i in this.squares2[j]) {
				var left = x - (this.square_size2 - this.min_square_size2) - this.left_offset2;
				var top = y - (this.square_size2 - this.min_square_size2) - this.top_offset2;

				var topv = top - (this.square_size2 * this.top_extra2);

				this.squares2[j][i].setAttribute('x',left);
				this.squares2[j][i].setAttribute('y',topv);

				if (this.grow_squares2) {
					this.squares2[j][i].setAttribute('width',this.square_size2);
					this.squares2[j][i].setAttribute('height',this.square_size2);
				}
				this.squares2[j][i].setAttribute('style','fill: '+this.colour4.get());
				x += this.square_size2 * 2;

				if (typeof(this.circles2[j]) !== 'undefined') {
					this.circles2[j][i].setAttribute('cx',left);
					this.circles2[j][i].setAttribute('cy',topv);
					this.circles2[j][i].setAttribute('r',this.circle_size2);
					this.circles2[j][i].setAttribute('fill',(this.circle_colour2 ? '#000' : '#fff'));
				}
			}
		...
		var thiz = this;
		setTimeout(function(){thiz.cycle2();}, this.speed2);
	}
}
