...
/*
			if (this.haz_spin) {
				if (this.spin_direction == 0) {
					//if (this.direction == 0) {
						this.dots[i]['degree'] += this.spin_speed; //Math.round(Math.random()*4);
...
/*			if (Math.round(Math.random()*30000) == 1) {
			...
			if (x >= window.innerWidth || x < -100 || y > window.innerHeight || y < -140) {
				...
				this.dots[i]['element'].innerHTML = String.fromCharCode(Math.random()*2000);
...
/*
			...
			if (this.character_mode == 0 && this.size_counter <= 0) {
				var size = ((this.dots[i]['size'] * this.dots[i]['distance'])/10);
				if (size >= 50) {
					size = 50;
				}
			...
			this.dots[i]['element'].style.fontSize = size;
